...
import { UPLOAD_AUDIO_DIR, UPLOAD_IMAGE_DIR, UPLOAD_VIDEO_DIR } from '~/constants/dir'
...
export const uploadAudioController = async (req: Request, res: Response) => {
  const url = await mediasService.uploadAudio(req)
...
export const serveAudioStreamController = (req: Request, res: Response) => {
  const range = req.headers.range
  if (!range) {
    return res.status(HTTP_STATUS.BAD_REQUEST).send('Requires Range header')
  }
  const { name } = req.params
  const audioPath = path.resolve(UPLOAD_AUDIO_DIR, name)
  ...
  // Dung lượng audio (bytes)
  const audioSize = fs.statSync(audioPath).size
  // DUng lượng audio cho mỗi phân đoạn stream
  const chunkSize = 30 * 10 ** 6 // 30MB
  // Lấy giá trị byte bắt đầu từ header Range (vd: bytes=1048576-)
  const start = Number(range.replace(/\D/g, ''))
  // Lấy giá trị byte kết thúc, vượt quá dung lượng audio thì lấy giá trị audioSize - 1
  const end = Math.min(start + chunkSize, audioSize - 1)

  // Dung lượng thực tế cho mỗi đoạn audio stream
  // THường đây sẽ là chunkSize, ngoại trừ đoạn cuối cùng
  const contentLength = end - start + 1
  const contentType = mime.getType(audioPath) || 'audio/*'

  /**
   * Format của header Content-Range: bytes <start>-<end>/<audioSize>
   * Ví dụ: Content-Range: bytes 1048576-3145727/3145728
   * Yêu cầu là `end` phải luôn luôn nhỏ hơn `audioSize`
  ...
  const audioSteams = fs.createReadStream(audioPath, { start, end })
  audioSteams.pipe(res)
}
